...
class GamessGenerator:
    def __init__(self, root):
        ...
        style.configure("TSpinbox", background=button_bg_color, font=("APTOS", 12), foreground="black")
        ...
        self.optInfo = {
            ...
            'scfmeth': 'RHF',
            'lvl': 'MPLEVL=2',
            'charge': '0',  # Set initial charge to 0
            'spin': '1',    # Set initial spin to 1
            'memory': '10',
            'memddi': '1',
            'basis': 'cc-pVDZ',
            'geom': ' ',
    ...
    def create_widgets(self):
        ...
        # Create and place labels and comboboxes or spinboxes
        for param, param_options in options_dict.items():
            label = ttk.Label(self.root, text=f"Choose {param.capitalize()}:")
            label.grid(row=row_counter, column=column_counter * 2, sticky=W, pady=2, padx=5)
            
            if param in ['charge', 'spin']:
                var = StringVar()
                entry = Entry(self.root, textvariable=var, width=15)
                entry.grid(row=row_counter, column=column_counter * 2 + 1, sticky=E+W, pady=2, padx=5)
                ...
                drop.grid(row=row_counter, column=column_counter * 2 + 1, sticky=E+W, pady=2, padx=5)
                setattr(self, f"{param}_var", var)
        ...
        self.preview.grid(row=row_counter, column=0, columnspan=6, sticky=N+E+S+W, pady=2, padx=5)
        ...
        generate_button.grid(row=row_counter, column=0, columnspan=6, sticky=N+E+S+W, pady=10, padx=5)
