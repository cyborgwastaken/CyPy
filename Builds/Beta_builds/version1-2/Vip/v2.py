...
class GamessGenerator:
    def __init__(self, root):
        ...
        style.configure("TCombobox", background=button_bg_color, font=("APTOS", 12), foreground="black", width=12)
    ...
    def create_widgets(self):
        ...
        for param, param_options in options_dict.items():
            ...
            label.grid(row=self.row_counter, column=0, sticky=W, pady=2, padx=(5, 2))  # Adjust padx to bring them closer
            var = StringVar()
            var.set(param_options[0])
            drop = ttk.Combobox(self.root, textvariable=var, values=param_options, state='readonly')
            drop.grid(row=self.row_counter, column=1, sticky=E+W, pady=2, padx=(2, 5))  # Adjust padx to bring them closer
        ...
        self.preview.grid(row=self.row_counter, column=0, columnspan=6, sticky=N+E+S+W, pady=5, padx=10)  # Align to the top, left, right, and bottom with some padding
        self.row_counter += 1
        ...
        generate_button.grid(row=self.row_counter, column=0, columnspan=6, sticky=N+E+S+W, pady=10, padx=10)  # Align to the top, left, right, and bottom with some padding
