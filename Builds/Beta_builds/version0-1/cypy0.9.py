...
class GamessGenerator:
    ...
    def create_widgets(self):
        ...
        for param, param_options in options_dict.items():
            ...
            label.grid(row=row_counter, column=column_counter * 2, sticky=W, pady=2, padx=(5, 2))  # Adjust padx to bring them closer
            var = StringVar()
            var.set(param_options[0])
            drop = ttk.Combobox(self.root, textvariable=var, values=param_options, state='readonly', width=15)
            drop.grid(row=row_counter, column=column_counter * 2 + 1, sticky=E+W, pady=2, padx=(2, 5))  # Adjust padx to bring them closer
